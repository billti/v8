...
Deoptimizer::Deoptimizer(Isolate* isolate, JSFunction function,
  ...
  if (compiled_code_.kind() == Code::OPTIMIZED_FUNCTION) {
    ...
    if (etw::v8Provider.IsEnabled(etw::kDeoptEvent)) {
      DeoptInfo deopt_info = GetDeoptInfo(compiled_code_, from_);
      DeoptimizationData deopt_data = DeoptimizationData::cast(compiled_code_.deoptimization_data());
      ...
      std::string deopt_reason{DeoptimizeReasonToString(deopt_info.deopt_reason)};
      std::string deopt_kind = deopt_kind_ == DeoptimizeKind::kSoft ? "Soft" : 
          (deopt_kind_ == DeoptimizeKind::kLazy ? "Lazy" : "Eager");

      Script::PositionInfo pos;
      if (!deopt_info.position.isInlined()) {
        SharedFunctionInfo function(SharedFunctionInfo::cast(deopt_data.SharedFunctionInfo()));
        if (function.script().IsScript()) {
          Script script = Script::cast(function.script());
          Object source_name = script.name();
          if (source_name.IsString()) src_name.assign(String::cast(source_name).ToCString().get());
          script.GetPositionInfo(deopt_info.position.ScriptOffset(), &pos, Script::WITH_OFFSET);
          line = pos.line + 1;
          column = pos.column + 1;
        }
      } else {
        InliningPosition inl = deopt_data.InliningPositions().get(deopt_info.position.InliningId());
        if (inl.inlined_function_id != -1) {
          SharedFunctionInfo sfi = deopt_data.GetInlinedFunction(inl.inlined_function_id);
          if (sfi.script().IsScript()) {
            ...
            line = pos.line + 1;
            column = pos.column + 1;
            fn_name = std::string{sfi.DebugName().ToCString().get()} + " <inlined in " + fn_name + ">";
          }
        }
      }

      etw::v8Provider.Deopt(deopt_reason, deopt_kind, src_name, fn_name, line, column);
    }
  }
