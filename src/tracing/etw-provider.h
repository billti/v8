...
#undef LIKELY
  ...
  void SetMetaDescriptors(EVENT_DATA_DESCRIPTOR* data_descriptor,
                          const void* metadata,
                          size_t size) {
    // Note: May be able to just set the name on the provider if only Win8 or
    ...
    EventDataDescCreate(data_descriptor, traits_.data(), static_cast<ULONG>(traits_.size()));
 ...
 protected:
  ...
  ~EtwProvider() { if (reg_handle_ != 0) EventUnregister(reg_handle_); }
